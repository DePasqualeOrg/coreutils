...
To find all annotated tests: `rg -n 'wasi_runner, ignore' tests/`

## Tools not yet covered by integration tests

arch, b2sum, cksum, csplit, date, dir, dircolors, fmt, join, ls, md5sum, mkdir, mv, nproc, pathchk, pr, printenv, ptx, pwd, readlink, realpath, rm, rmdir, seq, sha1sum, sha224sum, sha256sum, sha384sum, sha512sum, shred, sleep, split, tsort, uname, uniq, vdir
...
`is_safe_overwrite` (used by `cat` to detect input-is-output situations) assumes that overwriting is safe on WASI because wasmtime reports an inherited stdout file descriptor with zeroed device and inode values. That prevents a reliable identity comparison with the input. Tests that assert this error path are skipped.
